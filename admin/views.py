...
def roster_page(creators, error=None, message=None):
    ...
    body = (
        ...
        + "<a class='btn ghost' href='" + u("/roster/template") + "'>Download the template</a>"
        ...
        + "<p class='sub' style='margin-bottom:16px'>A spreadsheet cell cannot "
          "hold an image, so an import brings in everything except the photos. "
          "Select a whole folder of them here instead of opening each creator "
          "in turn.</p>"
        + "<p class='sub' style='margin-bottom:16px'>Each file is matched to a "
          "creator by its <strong>name</strong> — <code>HV-NA-001.jpg</code> by "
          "code, or <code>noha.mgdi.jpg</code> by handle. Anything that matches "
          "nothing is listed back rather than dropped, and the same 6MB and "
